...
select * from travel_request tr ;
select * from leave_request lr ;
select 
(select sum(tr.end_date-tr.start_date+1) from travel_request tr where tr.employee_id = 1) as banyak_cuti,
(select sum(lr.end_date-lr.start_date+1) from leave_request lr where lr.employee_id = 1) as banyak_dinas,

(select sum(tr.end_date-tr.start_date+1) from travel_request tr where tr.employee_id = 1) +
(select sum(lr.end_date-lr.start_date+1) from leave_request lr where lr.employee_id = 1) as tidak_dikantor;
